...
const serviceCategories: ServiceCategory[] = [
  {
    ...
    description: "Pricing packages for every budget",
    tiers: [
      {
        name: "Lump Sum",
        price: "1,000€",
    ...
    <Layout>
      ...
      {serviceCategories.map((category, categoryIndex) => (
        <section
          key={category.id}
          id={category.id}
          className={`py-14 md:py-18 ${categoryIndex % 2 === 1 ? "bg-muted/30" : "bg-background"}`}
        >
          <div className="container mx-auto px-4">
            ...
            >
              ...
              <p className="font-body text-muted-foreground text-lg max-w-2xl mx-auto">{category.description}</p>
            </motion.div>
            ...
            >
              {category.tiers.map((tier) => (
                ...
                >
                  {/* Tier Name */}
                  <h3 className="text-2xl font-bold text-foreground mb-4 uppercase tracking-wider">{tier.name}</h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-border">
                    <div className="mb-3">
                      ...
                      {tier.priceLabel && (
                        <span className="text-base font-body text-muted-foreground ml-2">{tier.priceLabel}</span>
                      )}
                    </div>
              ...
              {/* Custom CTA Card */}
              {category.showCustomCard && (
                <motion.div
                  variants={item}
                  className="bg-muted/50 border border-border rounded-2xl p-6 flex flex-col h-full"
                >
                  ...
                  <p className="font-body text-muted-foreground text-base mb-6 flex-grow">
                    Have a complex project or specific requirements? Let's discuss a custom solution tailored to your
                    needs.
                  </p>
                  <div className="mt-auto pt-4 border-t border-border">
                    <Button
                      asChild
                      variant="outline"
                      className="w-full border-navy text-navy hover:bg-navy hover:text-primary-foreground font-extrabold uppercase text-sm tracking-wider"
                    >
                      <Link to="/contact">
                        Reach Out
                        <ArrowRight className="w-4 h-4 ml-2" />
